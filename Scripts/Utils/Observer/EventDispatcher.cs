...
public class EventDispatcher : SingletonEvent<EventDispatcher>
{
    ...
    private void ConsoleOutput()
    {
        ...
        {
            Debug.Log($"Key: {kvp.Key} | with callback: {kvp.Value?.Target}");
        }
    }
    ...
    {
        isShowLog = true;
        Debug.Log("[Event] EventDispatcher active show debug");
    }
    ...
    public void ShowCallbackCounting()
    {
        foreach (KeyValuePair<EventID, Action<object>> kvp in _listeners)
        {
            Debug.Log($"=== Key: {kvp.Key} === | Count: {kvp.Value?.GetInvocationList().Length}");
            kvp.Value?.GetInvocationList().ToList().ForEach(x => Debug.Log($"Key: {kvp.Key} | Method: {x?.Method.Name} | Target: {x?.Target} "));
        }
    }
